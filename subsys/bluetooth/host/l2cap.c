...
static void l2cap_chan_rx_init(struct bt_l2cap_le_chan *chan)
{
	 ...
	 */
	uint16_t max_mps = MIN(chan->rx.mtu + BT_L2CAP_SDU_HDR_SIZE,
			       BT_L2CAP_RX_MTU);
	/* Use existing MPS if specified (by app) */
	if (chan->rx.mps) {
		chan->rx.mps = MIN(chan->rx.mps, max_mps);
	} else {
		chan->rx.mps = max_mps;
	}
...
static void l2cap_chan_send_credits_pdu(struct bt_l2cap_le_chan *chan, uint16_t credits)
{
	...
	struct net_buf *buf = l2cap_create_le_sig_pdu(BT_L2CAP_LE_CREDITS, get_ident(),
						      sizeof(*ev));
	if (!buf) {
...
int bt_l2cap_chan_recv_complete(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	if (bt_l2cap_chan_get_state(&le_chan->chan) == BT_L2CAP_CONNECTED) {
		...
		l2cap_chan_send_credits(le_chan, credits);
	}
				   ...
				   struct net_buf *buf, uint16_t seg)
{
	...
	if (bt_l2cap_chan_get_state(&chan->chan) == BT_L2CAP_CONNECTED) {
		l2cap_chan_send_credits(chan, seg);
	}

...
void seg_destroy(struct net_buf *buf);
...
void seg_destroy(struct net_buf *seg)
{
			       ...
			       struct bt_l2cap_le_chan *lechan)
{
	...
	view = bt_buf_make_view(view, sdu,
				seg_size, BT_L2CAP_CHAN_SEND_RESERVE,
				&get_seg_md(view)->view_meta);
...
void bt_l2cap_chan_del(struct bt_l2cap_chan *chan)
{
	...
	while (chan_has_data(le_chan)) {
		buf = net_buf_get(&le_chan->pdu_tx_queue, K_NO_WAIT);
		net_buf_unref(buf);
	}
...
void bt_l2cap_connected(struct bt_conn *conn)
{
	...
	STRUCT_SECTION_FOREACH(bt_l2cap_fixed_chan, fchan) {
		...
		k_fifo_init(&le_chan->pdu_tx_queue);
...
static void raise_data_ready(struct bt_l2cap_le_chan *le_chan)
{
	if (!atomic_set(&le_chan->_pdu_ready_lock, 1)) {
		sys_slist_append(&le_chan->chan.conn->upper_data_ready,
				 &le_chan->_pdu_ready);
...
static void cancel_data_ready(struct bt_l2cap_le_chan *le_chan)
{
	...
	sys_slist_find_and_remove(&conn->upper_data_ready,
				  &le_chan->_pdu_ready);
		      ...
		      bt_conn_tx_cb_t cb, void *user_data)
{
	...
	LOG_DBG("chan %p len %zu", le_chan, seg->len);
	...
	net_buf_put(&le_chan->pdu_tx_queue, seg);
...
static bool chan_has_data(struct bt_l2cap_le_chan *lechan)
{
	return !k_fifo_is_empty(&lechan->pdu_tx_queue);
}

struct net_buf *l2cap_data_pull(struct bt_conn *conn, size_t amount)
{
	sys_snode_t *pdu_ready = sys_slist_peek_head(&conn->upper_data_ready);
	 ...
	 * fragments.
	 */
	struct net_buf *pdu = k_fifo_peek_head(&lechan->pdu_tx_queue);
	...
	if (last_frag) {
		...
		struct net_buf *b = k_fifo_get(&lechan->pdu_tx_queue, K_NO_WAIT);
...
static struct net_buf *l2cap_chan_le_get_tx_buf(struct bt_l2cap_le_chan *ch)
{
	...
	return net_buf_get(&ch->sdu_tx_queue, K_NO_WAIT);
}
...
static void l2cap_chan_tx_init(struct bt_l2cap_le_chan *chan)
{
	...
	k_fifo_init(&chan->sdu_tx_queue);
	k_fifo_init(&chan->pdu_tx_queue);
...
static void l2cap_chan_destroy(struct bt_l2cap_chan *chan)
{
	...
	while ((buf = net_buf_get(&le_chan->sdu_tx_queue, K_NO_WAIT))) {
...
static void l2cap_chan_tx_resume(struct bt_l2cap_le_chan *ch)
{
	...
	if (!atomic_get(&ch->tx.credits) ||
	    (k_fifo_is_empty(&ch->sdu_tx_queue) && !ch->tx_buf)) {
...
static int bt_l2cap_dyn_chan_send(struct bt_l2cap_le_chan *le_chan, struct net_buf *buf)
{
	...
	net_buf_put(&le_chan->sdu_tx_queue, buf);

...
static void cap_eatt_mtu(struct bt_l2cap_le_chan *le_chan)
{
	if (le_chan->tx.mtu > le_chan->rx.mtu) {
		BT_DBG("chan %p (0x%04x): saturating TX MTU to RX MTU (%d)",
			le_chan, le_chan->tx.cid, le_chan->rx.mtu);
	}

	le_chan->tx.mps = MIN(le_chan->tx.mps, le_chan->rx.mps);
	le_chan->tx.mtu = MIN(le_chan->tx.mtu, le_chan->rx.mtu);
}
#endif

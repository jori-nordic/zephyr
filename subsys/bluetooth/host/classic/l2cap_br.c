...
static void raise_data_ready(struct bt_l2cap_br_chan *br_chan)
{
	if (!atomic_set(&br_chan->_pdu_ready_lock, 1)) {
		sys_slist_append(&br_chan->chan.conn->upper_data_ready,
				 &br_chan->_pdu_ready);
			...
			bt_conn_tx_cb_t cb, void *user_data)
{
	...
	net_buf_put(&br_chan->pdu_tx_queue, buf);
...
{
	return !k_fifo_is_empty(&br_chan->pdu_tx_queue);
}
...
struct net_buf *l2cap_br_data_pull(struct bt_conn *conn, size_t amount)
{
	 ...
	 * fragments.
	 */
	struct net_buf *pdu = k_fifo_peek_head(&br_chan->pdu_tx_queue);
	...
	if (last_frag) {
		...
		struct net_buf *b = k_fifo_get(&br_chan->pdu_tx_queue, K_NO_WAIT);
...
void bt_l2cap_br_chan_del(struct bt_l2cap_chan *chan)
{
	...
	while (chan_has_data(br_chan)) {
		buf = net_buf_get(&br_chan->pdu_tx_queue, K_NO_WAIT);
		net_buf_unref(buf);
	}

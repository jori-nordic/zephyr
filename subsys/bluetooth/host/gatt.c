...
#define STORE_DELAY	K_SECONDS(1)
...
void set_change_aware(struct bt_conn *conn, struct gatt_cf_cfg *cfg, bool aware)
{
	if (aware) {
			...
			const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	cfg->id = conn->id;
	set_change_aware(conn, cfg, true);
...
static int bt_gatt_store_cf(struct bt_conn *conn)
{
	...
	struct gatt_cf_cfg *cfg;
	char key[BT_SETTINGS_KEY_MAX];
	char dst[CF_NUM_BYTES + CF_FLAGS_NUM_BYTES];
	char *str;
	size_t len;
	int err;
	...
	if (!cfg) {
		...
		/* add the data to a temp variable */
		memcpy(dst, str, len);

		/* add the change-aware flag */
		bool change_aware = atomic_test_bit(cfg->flags, CF_CHANGE_AWARE);
		LOG_ERR("change aware? %u", change_aware);

		dst[len] = (uint8_t)(change_aware ? 1U : 0);
		len += CF_FLAGS_NUM_BYTES;
...
static void clear_ccc_cfg(struct bt_gatt_ccc_cfg *cfg)
{
	...
	defined(CONFIG_BT_SETTINGS_CF_STORE_ON_WRITE)

/* Does it make sense to actually use a kernel queue?
 * The whole conn index business is iffy at best
 * -> will use 4 bytes bookkeeping per-el
 * -> freeing will be harder */
...
static bool gatt_delayed_store_is_last_queued(struct conn_list *el, enum delayed_store_flags flag)
{
	return atomic_get(el->flags) == 1 << flag;
}
...
static void gatt_delayed_store_dequeue(struct bt_conn *conn, enum delayed_store_flags flag)
{
	...
	if (el->conn != NULL) {
		if (gatt_delayed_store_is_last_queued(el, flag)) {
...
static void gatt_delayed_store_enqueue(struct bt_conn *conn, enum delayed_store_flags flag)
{
	...
	if (!queued && bonded) {
		...
		k_work_reschedule(&gatt_delayed_store.work, STORE_DELAY);
	}
}
...
#endif	/* CONFIG_BT_SETTINGS_CCC_STORE_ON_WRITE ||
	 * CONFIG_BT_SETTINGS_CF_STORE_ON_WRITE
	 */
		  ...
		  void *cb_arg)
{
	...
	if (len_rd) {
		...
		uint8_t change_aware = 0;

		len = read_cb(cb_arg, &change_aware, sizeof(change_aware));
		if (len < 0) {
			LOG_DBG("Change-aware status not stored (err %zd)", len);
			return len;
		}
		/* TODO: check this doens't trigger a useless write to FLASH */
		LOG_ERR("recall change-awareness");
		/* set_change_aware(conn, cfg, change_aware == true); */
		atomic_set(cfg->flags, (change_aware & 1) << CF_CHANGE_AWARE);

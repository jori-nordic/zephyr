...
/* TODO: could we add a FRAG_ALREADY flags instead of the extra userdata? */
enum {
	FRAG_START,
	FRAG_CONT,
	FRAG_SINGLE,
...
static struct net_buf *create_frag(struct bt_conn *conn, struct net_buf *buf)
{
	...
	/* TODO: could we point to buffer memory directly?
...
static int send_buf(struct bt_conn *conn, struct net_buf *buf)
{
	...
	LOG_HEXDUMP_DBG(buf->data, buf->len, "buf@entry");
	...
	while (buf->len > conn_mtu(conn)) {
		...
		int err = send_frag(conn, frag, flags, false);
		if (err) {
			       ...
			       struct k_poll_event *events)
{
	...
	/* Here's an idea:
	 * - put a K_NO_WAIT for the semaphore acquisition
	 * - remove the queue from the k_poll
	 * - add the NCP event to the k_poll
	 * -> this way this process_tx won't get triggered all the gdamn time
	 * */

	bool buffer_available = k_sem_count_get(bt_conn_get_pkts(conn)) > 0;
	if (buffer_available) {
...
void bt_conn_process_tx(struct bt_conn *conn)
{
	...
	if (err == -EIO) {
		...
		/* FIXME: check if it's really ok to do this */

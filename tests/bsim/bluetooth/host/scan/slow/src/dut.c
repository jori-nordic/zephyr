...
static void validate_data(struct net_buf_simple *ad)
{
	...
	TEST_ASSERT(peer_ad->type == type,
		    "Type mismatch: expect %x got %x", peer_ad->type, type);
	TEST_ASSERT(peer_ad->data_len + 1 == len,
		    "Data len mismatch: expect %x got %x", peer_ad->data_len + 1, len);
			 ...
			 struct net_buf_simple *ad)
{
	...
	LOG_DBG("Device found: %s (RSSI %d), type %u, AD data len %u", addr_str, rssi, type, ad->len);

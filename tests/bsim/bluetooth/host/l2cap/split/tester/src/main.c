...
typedef void* bt_conn_tx_cb_t;	/* TODO: fix that */
...
static void handle_meta_event(struct net_buf *buf)
{
	...
	switch (code) {
		...
		case BT_HCI_EVT_LE_CHAN_SEL_ALGO:
			...
			break;
		default:
			LOG_DBG("unhandled meta event %x", code);
...
static void handle_ncp(struct net_buf *buf)
{
	...
	while (count--) k_sem_give(&acl_pkts);
}
...
static void handle_l2cap_credits(struct net_buf *buf)
{
	...
	while (credits--) k_sem_give(&tx_credits);
}
...
static void handle_l2cap_connected(struct net_buf *buf)
{
	...
	LOG_ERR("l2cap connected: mtu %d mps %d credits: %d",
		mtu, mps, credits);
...
static void handle_sig(struct net_buf *buf)
{
	...
	switch (hdr->code) {
		...
		case BT_L2CAP_LE_CREDITS:
			...
			return;
		default:
			LOG_ERR("unhandled opcode %x", hdr->code);
			return;
	}
}
...
static void recv(struct net_buf *buf)
{
	...
	LOG_HEXDUMP_DBG(buf->data, buf->len, "HCI RX (not data/event)");
	net_buf_unref(buf);
}
...
static void write_default_data_len(uint16_t tx_octets, uint16_t tx_time)
{
	...
	struct net_buf *buf = bt_hci_cmd_create(BT_HCI_OP_LE_WRITE_DEFAULT_DATA_LEN,
						sizeof(*cp));
	__ASSERT_NO_MSG(buf);
...
static void send_l2cap_packet(struct net_buf *buf, uint16_t cid)
{
	...
	/* Always full packets, no HCI fragmentation */
...
static void send_l2cap_sdu(uint8_t *data, uint16_t sdu_len)
{
	uint16_t pdu_len;
	int mps = 5;
	...
	for (int i=0; sdu_len; i++) {
		...
		if (i==0) {
			net_buf_push_le16(buf, sdu_len); /* add SDU len */
		...
		data = &data[pdu_len];
		sdu_len = MAX(sdu_len - pdu_len, 0);

		LOG_ERR("send PDU %d sdu_len %d", i, sdu_len);
		LOG_HEXDUMP_WRN(buf->data, buf->len, "PDU");
...
void test_procedure_0(void)
{
	...
	send_l2cap_sdu(data, sizeof(data));

...
static void handle_l2cap_connected(struct net_buf *buf)
{
	...
	/* TODO: what about static channels */
...
static void handle_att_notification(struct net_buf *buf)
{
	...
	LOG_ERR("Got notification for 0x%04x len %d", handle, buf->len);
	LOG_HEXDUMP_INF(buf->data, buf->len, "payload");
}
...
static void handle_att_write(struct net_buf *buf)
{
	...
	LOG_ERR("Got write for 0x%04x len %d", handle, buf->len);
	LOG_HEXDUMP_INF(buf->data, buf->len, "payload");
}
...
static void handle_l2cap(struct net_buf *buf)
{
	...
	__ASSERT(buf->len == hdr->len, "buflen = %d != hdrlen %d",
		 buf->len, hdr->len);
...
void test_procedure_0(void)
{
	...
	LOG_INF("inform central of our RX MTU");
	struct net_buf *buf = alloc_l2cap_pdu();
	net_buf_add_u8(buf, BT_ATT_OP_MTU_REQ);
	net_buf_add_le16(buf, CONFIG_BT_BUF_ACL_RX_SIZE - 4);	/* FIXME */

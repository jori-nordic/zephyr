...
static void send_update_conn_params_req(struct bt_conn *conn)
{
	...
	if (atomic_test_bit(conn_info_ref->flags, CONN_INFO_CONN_PARAMS_UPDATED) == false) {
		...
		if (err) {
			FAIL("Updating connection parameters failed (%d) %s", err, addr);
			return;
		}
...
static void connected(struct bt_conn *conn, uint8_t conn_err)
{
	...
	if (conn_info_ref->security_level < BT_SECURITY_L2) {
		...
		if (!err) {
			TERM_SUCCESS("Security level for connection %p with peer %s is set to : %u",
				     conn, addr, BT_SECURITY_L2);
		} else {
			FAIL("Failed to set security for conn %p with peer %s (%d)", conn,
				  addr, err);
		}
	}
#endif /* CONFIG_BT_SMP */
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	TERM_WARN("Connection (%d) @ %p with Peer %s terminated (reason 0x%02x)", conn_ref_index,
		  conn, addr, reason);
	ASSERT(reason == BT_HCI_ERR_REMOTE_POWER_OFF, "Disconnected (reason 0x%02x)", reason);
	bt_conn_unref(conn);
...
static void security_changed(struct bt_conn *conn, bt_security_t level, enum bt_security_err err)
{
	...
	if (err) {
		k_panic();
		FAIL("Security for %p failed: %s level %u err %d", conn, addr, level, err);
		return;
	}
